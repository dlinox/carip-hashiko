...
class Login extends CI_Controller
{
	...
	function index()
	{
		...
		*/
		...
		$this->load->view($this->router->fetch_class() . '/form', $captcha);
	...
	function verificar_login()
	{
		...
		if (isset($_COOKIE["captcha_count"])) {
			if ($_COOKIE["captcha_count"] > 3) {
				...
				$dts = array(
					"word" => $captcha, "ip_address" => $ip, "captcha_time >" => $expira
				);
		...
		if ($success_captcha != FALSE) {
			if ($usuario != FALSE) {
				...
				if ($usua != FALSE) {
					$usu_pas = array(
						"user_usua" => $usuario,
						"password_usua" => md5($password),
						"habilitado_usua" => '1'
					);

					if ($this->Model_login->login($usu_pas, TRUE) != FALSE) {
						$cambios = array("intento_usua" => '0');
	...
	function bloquear($contador, $id, $ip)
	{
		...
		$cambios = array("intento_usua" => $contador + 1, "ultimoip_usua" => $ip);
		if ($cambios["intento_usua"] >= 6) {
			$cambios = array("habilitado_usua" => '0', "ultimoip_usua" => $ip);
		}
